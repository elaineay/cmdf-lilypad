...
SELECT_SQL = ''' select * from locations where type='{type}' and cost='{cost}' limit 5 '''
...
def get_locations(connection, type, cost):
    ...
    with connection.cursor() as cursor:
        cursor.execute(SELECT_SQL.format(type=type, cost=cost))
